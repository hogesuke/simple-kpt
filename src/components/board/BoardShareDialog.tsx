...
export function BoardShareDialog({ boardId, isOpen, onOpenChange }: BoardShareDialogProps): ReactElement {
  ...
  }, [handleError, shareUrl, t]);

  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">{t('ボードを共有')}</DialogTitle>
          <DialogDescription>
            {t('ボードが作成されました。')}
            <br />
            {t('URLを共有して、ボードにメンバーを招待しましょう。')}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            />
            <Button size="sm" variant="outline" onClick={handleCopy} aria-label={copied ? t('コピー済み') : t('URLをコピー')}>
